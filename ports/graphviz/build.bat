...
if not defined PKG_CONFIG_PREFIX set PKG_CONFIG_PREFIX=%_PREFIX%
...
cmake -G "Ninja"                                                               ^
  ...
  -DCMAKE_C_STANDARD_LIBRARIES="pcrt.lib Ole32.lib Gdi32.lib User32.lib"       ^
  -DCMAKE_CXX_COMPILER=cl                                                      ^
  -DCMAKE_CXX_FLAGS="-EHsc %C_OPTS% %C_DEFS%"                                  ^
  -DCMAKE_CXX_STANDARD_LIBRARIES="pcrt.lib Ole32.lib Gdi32.lib User32.lib"     ^
...
sed -e "s#\([A-Za-z]\):/\([^/]\)#/\L\1\E/\2#g" -i libcdt.pc
sed -e "s#\([A-Za-z]\):/\([^/]\)#/\L\1\E/\2#g" -i libcgraph.pc
sed -e "s#\([A-Za-z]\):/\([^/]\)#/\L\1\E/\2#g" -i libgvc.pc
sed -e "s#\([A-Za-z]\):/\([^/]\)#/\L\1\E/\2#g" -i libgvpr.pc
sed -e "s#\([A-Za-z]\):/\([^/]\)#/\L\1\E/\2#g" -i libpathplan.pc
sed -e "s#\([A-Za-z]\):/\([^/]\)#/\L\1\E/\2#g" -i libxdot.pc
popd
exit /b 0
