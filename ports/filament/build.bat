...
rem FIXME:
rem 1. use option BUILD_SHARED_LIBS for filament. some subprojects
rem    under third_party need to be updated to support it, e.g. basisu
cmake -G "Ninja"                                                               ^
...
if not exist "%PREFIX%\share\doc\%PKG_NAME%" (
  mkdir "%PREFIX%\share\doc\%PKG_NAME%"
)
move "%PREFIX%\docs" "%PREFIX%\share\doc\%PKG_NAME%"
move "%PREFIX%\lib\x86_64" "%PREFIX%\lib"
move "%PREFIX%\LICENSE" "%PREFIX%\share\doc\%PKG_NAME%\LICENSE"
move "%PREFIX%\README.md" "%PREFIX%\share\doc\%PKG_NAME%\README.md"
exit /b 0
