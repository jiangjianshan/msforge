...
cd "%BUILD_DIR%" && nmake clean
exit /b 0

:configure_stage
echo "Configuring %PKG_NAME% %PKG_VER%"
cd "%BUILD_DIR%"
set KRB_INSTALL_DIR=%PREFIX%
nmake /K -f Makefile.in prep-windows || exit 1
exit /b 0

:build_stage
echo "Building %PKG_NAME% %PKG_VER%"
cd "%BUILD_DIR%"
nmake /K NODEBUG=1 NO_LEASH=1 || exit 1
exit /b 0

:install_stage
echo "Installing %PKG_NAME% %PKG_VER%"
cd "%BUILD_DIR%"
nmake /K install NODEBUG=1 NO_LEASH=1 || exit 1
exit /b 0
