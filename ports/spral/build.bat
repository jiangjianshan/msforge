...
set CU_OPTS=-std=c++17 -Wno-deprecated-gpu-targets -gencode arch=compute_%NV_COMPUTE:.=%,code=sm_%NV_COMPUTE:.=% -Xcompiler=-MD
...
if exist "%CUDA_PATH%" (
  ...
  set cuda_flags=-Dcuda_args="%CU_OPTS%" -Dcuda_link_args="-shared"
)
meson setup "%BUILD_DIR%"                                                      ^
  ...
  -Dc_winlibs="pcrt.lib,pthread.lib"                                           ^
  -Dcpp_winlibs="pcrt.lib,pthread.lib"                                         ^
  -Dfortran_args="%F_OPTS%" %cuda_flags%                                       ^
