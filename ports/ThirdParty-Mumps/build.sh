...
configure_stage()
{
  ...
  #    '*cl | cl.exe'
  # 3. To use '$SRC_DIR/compile ifort' instead of 'ifort', has solved the
  #    following issue:
  #
  #    libifcoremt.lib(for_main.obj) : error LNK2019: unresolved external symbol MAIN__ referenced in function main
  #
  #    The above issue is because only 'ifort', it can't handle '-l' option. So that
  #    it can't link the dependencies needed for this library
  # 4. Don't set '--enable-relocatable', otherwise the key 'prefix' in .pc will be
  #    'prefix=${pcfiledir}/../..'
  ...
  F77="$SRC_DIR/compile ifort"                                                                               \
  FFLAGS="-f77rtl $F_OPTS"                                                                                   \
  FC="$SRC_DIR/compile ifort"                                                                                \
  FCFLAGS="$F_OPTS"                                                                                          \
  LD="link -nologo"                                                                                          \
