...
cd "%BUILD_DIR%"
nmake clean CCTYPE=MSVC143 INST_DRV= INST_TOP="%PREFIX%" INST_HTML="%PREFIX%\share\html"
exit /b 0

:build_stage
echo "Building %PKG_NAME% %PKG_VER%"
cd "%BUILD_DIR%"
nmake /K CCTYPE=MSVC143 INST_DRV= INST_TOP="%PREFIX%" INST_HTML="%PREFIX%\share\html"
exit /b 0

:install_stage
echo "Installing %PKG_NAME% %PKG_VER%"
cd "%BUILD_DIR%"
nmake /K install CCTYPE=MSVC143 INST_DRV= INST_TOP="%PREFIX%" INST_HTML="%PREFIX%\share\html"
exit /b 0
