...
BUILD_DIR=$SRC_DIR/Alps/build${ARCH//x/}
C_OPTS='-diagnostics:column -experimental:c11atomics -fp:precise -MD -nologo -openmp:llvm -utf-8'
C_DEFS='-DWIN32 -D_WIN32_WINNT=_WIN32_WINNT_WIN10 -D_CRT_DECLARE_NONSTDC_NAMES -D_CRT_SECURE_NO_DEPRECATE -D_CRT_SECURE_NO_WARNINGS -D_CRT_NONSTDC_NO_DEPRECATE -D_CRT_NONSTDC_NO_WARNINGS -D_USE_MATH_DEFINES -DNOMINMAX -DALPS_BUILD'
...
prepare_stage()
{
  ...
  WANT_AUTOCONF='2.72' WANT_AUTOMAKE='1.17' ./run_autotools $SRC_DIR/Alps
  cd "$SRC_DIR"
  rm -rfv autom4te.cache Alps/autom4te.cache
...
configure_stage()
{
  ...
  # 4. Set to '-std:c++17' or '-std:c++20' will throw out issue:
