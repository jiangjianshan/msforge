...
cd "%SRC_DIR%"
python -m pip install --upgrade -r scripts/basic.requirements.txt
...
sed -e "s#\([A-Za-z]\):/\([^/]\)#/\L\1\E/\2#g" -i mbedcrypto.pc
sed -e "s#\([A-Za-z]\):/\([^/]\)#/\L\1\E/\2#g" -i mbedtls.pc
sed -e "s#\([A-Za-z]\):/\([^/]\)#/\L\1\E/\2#g" -i mbedx509.pc
popd
exit /b 0
