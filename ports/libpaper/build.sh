...
prepare_stage()
{
  ...
  # NOTE:
  # 1. Fix following issues:
  # paper-paper.obj : error LNK2019: unresolved external symbol paper_lineno referenced in function paper_die
  # paper-paper.obj : error LNK2019: unresolved external symbol paper_specsfile referenced in function paper_die
  pushd lib || exit 1
  sed                                                                                    \
    -e 's|extern size_t paper_lineno|size_t paper_lineno|g'                              \
    -e 's|extern char \*paper_specsfile|char *paper_specsfile|g'                         \
    -i paper.h
  popd || exit 1
}
...
patch_stage()
{
  ...
  cp -v "$SRC_DIR/man/paper.x" "$BUILD_DIR/man/paper.x"
  cp -v "$SRC_DIR/man/paperspecs.5.in" "$BUILD_DIR/man/paperspecs.5.in"
  pushd man || exit 1
  sed -e 's|\$(srcdir)/\$@\.in|$@.in|g' -i Makefile
  popd || exit 1
}
