...
set C_DEFS=-DWIN32 -D_WIN32_WINNT=_WIN32_WINNT_WIN10 -D_CRT_DECLARE_NONSTDC_NAMES -D_CRT_SECURE_NO_DEPRECATE -D_CRT_SECURE_NO_WARNINGS -D_CRT_NONSTDC_NO_DEPRECATE -D_CRT_NONSTDC_NO_WARNINGS -D_USE_MATH_DEFINES -DNOMINMAX -DFREEGLUT
...
for %%f in ("libwebp*.pc") do (
  sed -e "s#\([A-Za-z]\):/\([^/]\)#/\L\1\E/\2#g" -i "%%~f"
)
sed -e "s#\([A-Za-z]\):/\([^/]\)#/\L\1\E/\2#g" -i libsharpyuv.pc
popd
exit /b 0
