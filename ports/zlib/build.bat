...
cd "%SRC_DIR%"
rem Fix wrong .pc installation location
sed -e "s|share\/pkgconfig|lib\/pkgconfig|g" -i CMakeLists.txt
exit /b 0
...
if not exist "%PREFIX%\lib\zdll.lib" (
  mklink "%PREFIX%\lib\zdll.lib" "%PREFIX%\lib\zlib.lib"
)
if not exist "%PREFIX%\lib\z.lib" (
  mklink "%PREFIX%\lib\z.lib" "%PREFIX%\lib\zlib.lib"
)
if not exist "%PREFIX%\lib\libz.lib" (
  mklink "%PREFIX%\lib\libz.lib" "%PREFIX%\lib\zlibstatic.lib"
)
pushd "%PREFIX%\lib\pkgconfig"
sed -e "s#\([A-Za-z]\):/\([^/]\)#/\L\1\E/\2#g" -i zlib.pc
popd
exit /b 0
