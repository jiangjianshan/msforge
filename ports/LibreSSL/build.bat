...
rem NOTE: LibreSSL is not compatible for OpenSSL, so that need to set bin, include and
rem       lib directory
cmake -G "Ninja"                                                               ^
  ...
  -DCMAKE_INSTALL_BINDIR="%PREFIX%/bin/%PKG_NAME%"                             ^
  -DCMAKE_INSTALL_INCLUDEDIR="%PREFIX%/include/%PKG_NAME%"                     ^
  -DCMAKE_INSTALL_LIBDIR="%PREFIX%/lib/%PKG_NAME%"                             ^
  -DCMAKE_POLICY_DEFAULT_CMP0117=OLD                                           ^
  -DLIBRESSL_INSTALL_CMAKEDIR="%PREFIX%/lib/cmake/%PKG_NAME%"                  ^
  -DLIBRESSL_TESTS=OFF                                                         ^
  .. || exit 1
exit /b 0
