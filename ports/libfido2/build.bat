...
set C_OPTS=-nologo -MD -diagnostics:column -wd4211 -wd4005 -wd4819 -wd4996 -fp:precise -openmp:llvm -utf-8 -Zc:__cplusplus -experimental:c11atomics
...
if not defined CBOR_PREFIX set CBOR_PREFIX=%_PREFIX%
if not defined LIBRESSL_PREFIX set LIBRESSL_PREFIX=%_PREFIX%
...
cmake -G "Ninja"                                                               ^
  ...
  -DBUILD_EXAMPLES=OFF                                                         ^
  -DNFC_LINUX=OFF                                                              ^
	-DCBOR_BIN_DIRS="!CBOR_PREFIX!/bin"                                          ^
	-DCBOR_INCLUDE_DIRS="!CBOR_PREFIX!/include"                                  ^
  -DCBOR_LIBRARY_DIRS="!CBOR_PREFIX!/lib"                                      ^
	-DCRYPTO_BIN_DIRS="!LIBRESSL_PREFIX!/bin/LibreSSL"                           ^
	-DCRYPTO_INCLUDE_DIRS="!LIBRESSL_PREFIX!/include/LibreSSL"                   ^
  -DCRYPTO_LIBRARY_DIRS="!LIBRESSL_PREFIX!/lib/LibreSSL"                       ^
	-DZLIB_BIN_DIRS="!ZLIB_PREFIX!/bin"                                          ^
	-DZLIB_INCLUDE_DIRS="!ZLIB_PREFIX!/include"                                  ^
  -DZLIB_LIBRARY_DIRS="!ZLIB_PREFIX!/lib"                                      ^
  -DZLIB_LIBRARIES="!ZLIB_PREFIX!/lib/zlib.lib"                                ^
  .. || exit 1
exit /b 0
...
sed -e "s#\([A-Za-z]\):/\([^/]\)#/\L\1\E/\2#g" -i libfido2.pc
popd
exit /b 0
